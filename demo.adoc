...
. Run `npx create-react-app react-app`; show app with `npm start`

. Add TypeScript support with `yarn add -D typescript @types/node @types/react @types/react-dom @types/jest` [`cra-ts`]
...
. Fix `Promise` type error by modifying `tsconfig.json` and adding a "lib" key/value for ES6.
...
. Update `App.tsx` to have Okta config and use `<Security/>` component [`react-config`, `react-security`]
